...
from rosetta.model.projector_v3 import Projector
...
class RosettaModel(nn.Module):
    ...
    @staticmethod
    def _uses_source_hidden_state(projector: Projector) -> bool:
        return bool(getattr(projector, "uses_source_hidden_state", False))
    ...
    def _source_requires_hidden_states(self, source_model_idx: int) -> bool:
        ...
        for _, entry in self.projector_dict[self.base_model_idx][source_model_idx].items():
            for _, projector_idx in entry:
                if self._uses_source_hidden_state(self.projector_list[projector_idx]):
                    return True
        return False

    @staticmethod
    def _select_source_hidden_state(hidden_states, source_layer_idx: int, expected_length: int) -> torch.Tensor:
        if hidden_states is None:
            raise ValueError("source hidden states were not returned by the source model")
        if source_layer_idx >= len(hidden_states):
            raise IndexError(
                f"source_layer_idx {source_layer_idx} is out of range for "
                f"{len(hidden_states)} hidden-state tensors"
            )
        # For a decoder block, hidden_states[layer_idx] is the block input: the
        # same representation that produces that layer's K/V projections.
        source_hidden = hidden_states[source_layer_idx]
        if source_hidden.size(1) == expected_length:
            return source_hidden
        return source_hidden[:, -expected_length:, :]
    ...
    def register_hooks(self, input_ids, attention_mask, position_ids, base_kv_cache, source_model_idx, source_kv_cache):
        ...
        for target_layer_idx, entry in self.projector_dict[self.base_model_idx][source_model_idx].items():
            ...
            for source_model_layer_idx, projector_idx in pair_list:
                ...
                if self._uses_source_hidden_state(projector):
                    new_source_hidden = self._select_source_hidden_state(
            ...
            if i == num_sections - 1:
                    ...
                    try:
                        ...
                        with torch.no_grad():
                            ...
                            if source_requires_hidden:
                                source_hidden_state_dict[source_model_idx] = out.hidden_states
                ...
                if self.base_model_idx in self.projector_dict:
                    ...
                    if sharer_mask > 0:
                        ...
                        for source_model_idx in self.projector_dict[self.base_model_idx].keys():
                            ...
                            for target_layer_idx, entry in self.projector_dict[self.base_model_idx][source_model_idx].items():
                                ...
                                for source_model_layer_idx, projector_idx in pair_list:
                                    projector = self.projector_list[projector_idx]
                                    if self._uses_source_hidden_state(projector):
                                        new_source_input = self._select_source_hidden_state(
                                            source_hidden_state_dict.get(source_model_idx),
                                            source_model_layer_idx,
                                            end - start,
                                        )
                                    else:
                                        ...
                                        new_source_input = (new_source_key_cache, new_source_value_cache)
                                    torch.cuda.nvtx.range_push("projector")
                                    projected_key, projected_value = projector.forward(
                                        new_source_input,
                                        new_base_kv_cache
                                    )
                                    torch.cuda.nvtx.range_pop()
                                    projected_kv_list.append((projected_key, projected_value)) #float32
                                    source_kv_list.append(new_source_input)

                                # Use first projector result
                                torch.cuda.nvtx.range_push("accumulate")
                                agg_key, agg_value = projected_kv_list[0]
                        ...
                        torch.cuda.nvtx.range_push("apply_cache")
